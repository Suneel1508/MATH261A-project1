...
date: "09/21/2025"
abstract: "This paper looks at two simple signals to explain where a driver finishes in modern Formula 1: how far their best lap is from the race’s fastest lap (pace deficit) and where they start on the grid. Using the F1DB CSVs from 2014 to the present, I fit one-variable models for each signal and compare how well they line up with finishing position. The results show a clear, steady link for starting grid position and a meaningful—though noisier—link for pace deficit. Together, these findings show how much we can learn from a single number, while noting that race strategy and incidents still play a big role."
...
I use ordinary least squares via `lm()` in R[@R-base-2024]. These one-variable models assume a roughly linear average effect and approximately constant spread of residuals; I’ll check standard residual plots in Results. Because we use just one predictor at a time, the goal is clarity: quantify how much **one simple number** (pace deficit or grid slot) moves finishing position, in **positions per second** or **positions per grid place**, while acknowledging unmodeled race factors.
